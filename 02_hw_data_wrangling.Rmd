...
library(dplyr)
```
# There are 7,014 rows, so there are 7,014 candidates. 
...
```{r}
party_incumbent_promote <- fb_ads_housesenate |>
  group_by(party, incumbency) |>
  summarize(promote_prop = mean(ad_tone_promote, na.rm = TRUE)) |>
  arrange(party, incumbency) |> as_tibble()
party_incumbent_promote
```
```{r eval = FALSE}
knitr::kable(party_incumbent_promote, digits = 3, col.names = c("Party", "Incumbency", "Avg Promoting Proportion"))
```

# Based on the data, Democratic incumbents sponsored the most promoting ads on average. 
...
```{r}
fb_ads_millions <- fb_ads |> 
  mutate(impressions_millions = impressions / 1000000)
```

```{r}
plot_q3 <- fb_ads_millions |>
  filter(office == "US House") |>
  ggplot(aes(x=impressions_millions)) +
  geom_histogram(binwidth = 1) +
  labs(x = "Impressions (in millions)", y = "Number of Candidates")
print(plot_q3)
```

# The graph is heavily skewed to the right. Most of the house candidates had less than 10 million ad impressions on Facebook. 

\newpage
ofi## Question 4 (13 points)
